...
In this paper, we identify and address this gap by proposing two complementary strategies: side effect avoidance, which steers control flow along cleaner paths, and side effect neutralization, which lets residual effects execute safely through primitive-aware object matching or pointer materialization.
We present SyzPass, an automated framework that identifies and resolves side effects using high-sensitivity vulnerability context exploration, symbolic-execution-guided path exploration, and primitive-aware matching. Evaluated on 342 real-world Linux kernel bugs, SyzPass reports strict primitive-aware results for 97 cases with complete primitive-aware records. Under this strict accounting, SyzPass provides actionable side-effect neutralization strategies for 14 primitive instances from 10 bugs, and its guidance enables the construction of three privilege-escalation exploits.
