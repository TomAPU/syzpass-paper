...
As we can see from the simplified syscall implementations in Figure~\ref{fig:motivating_code}, an attacker can invoke \texttt{syscall\_A()} to potentially trigger 3 primitives: UAF Write (an OOB/UAF Write, OUW, in our taxonomy), Arbitrary Address Write (AAW) and Control-Flow Hijack (CFH). On the other hand, invoking \texttt{syscall\_B()} will potentially trigger 2 primitives: AAW and CFH.
...
%Here, \emph{AAW-type residual effect} denotes an undesirable effect in which a pointer corrupted by sprayed data is dereferenced (the attacker may not know a valid address to place in that pointer field).
