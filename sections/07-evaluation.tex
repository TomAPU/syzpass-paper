...
Among them, 1,048 are usable, and 372 are not usable.
Notably, SyzPass accounts for 206 of the 1,048 usable instances (74 by side-effect avoidance and 132 by side-effect neutralization), indicating that a substantial fraction of usability arises only after explicitly reasoning about and mitigating side effects.

If we focus on higher-risk primitives (OUW, AAW, CFH, and IF), the result reflects our exploit-aware model: AAW, CFH, and symbolic-argument IF are counted as clean because address disclosure and kernel-memory control make object matching unnecessary for those primitives. SyzPass still accounts for 75 of 446 usable high-risk primitives. This impact is concentrated in primitives that still require field/layout matching: SyzPass accounts for 62 of 146 usable OUW primitives and 13 of 51 usable IF primitives.
...
At the bug level, all 115 bugs contain at least one usable primitive instance.
Moreover, 33 bugs benefit from SyzPass via avoidance and/or neutralization, including 15 benefiting from avoidance and 27 benefiting from neutralization (9 benefit from both). For the remaining 82 bugs, 64 contain only clean primitive instances and thus do not require avoidance or neutralization. Another 18 bugs contain either clean or non-usable instances.
