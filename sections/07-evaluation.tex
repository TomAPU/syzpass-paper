...
\begin{table}[t]
  ...
  \begin{tabular}{@{}p{0.43\linewidth}r p{0.32\linewidth}@{}}
    ...
    Primitive-aware analyses & 97 & Counted cases \\
    Counted primitive sites & 781 & Main sites \\
    \bottomrule
  \end{tabular}
  \caption{Evaluation dataset funnel from stable Syzbot memory-corruption bugs to primitive-aware analyses.}
  \label{tab:dataset-funnel}
\end{table}

\noindent\textbf{Reading the counts.} The funnel is 342 stable bugs $\rightarrow$ 115 symbolically reachable bugs $\rightarrow$ 97 primitive-aware analyses $\rightarrow$ 781 primitive sites. These analyses form the basis for primitive-aware matching under the two capability models.
...
Table~\ref{tab:primitive-usability-breakdown} reports the same primitive sites under PM and OM. Sites are evaluated by the primitive-specific rules in Table~\ref{tab:primitive-aware-rules}. PM captures the stronger pointer-materialization model, while OM reports the portable object-matching subset.

\input{generated/primitive_aware_breakdown}



Overall, we observe 781 primitive instances across the 97 primitive-aware analyses. Under PM, 291 sites are candidates, spanning 63 of 97 analyzed bugs and including 166 AAW, 83 CFH/FPD, and 38 IF sites. Even without PM, the portable OM subset contains 15 candidates: 1 clean site and 14 SyzPass-attributed candidates. This gap highlights how stronger pointer materialization changes the reachable primitive space while preserving object matching as the deployment-independent baseline.
