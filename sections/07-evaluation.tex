...
\noindent\textbf{RQ2. How does SyzPass guidance support expert exploit development?}
...
\noindent\textbf{Threat model and accounting.} We report two capability views. \emph{Object matching} (OM) is the portable baseline: the attacker can trigger the bug, control syscall and spray data, and reclaim vulnerable memory with sprayable objects, while residual effects must fit real object layouts. \emph{Pointer materialization} (PM) additionally permits address disclosure and kernel-addressable controlled memory for pointer-related primitives. PM is a sensitivity view over stronger exploit capabilities, while the case studies show how reported paths, constraints, and object matches are consumed by manual exploit development. A bug enters the aggregate when its record contains primitive-aware metadata for at least one counted site. Individual AVW/CVW sites without determined destination fields are reported separately because strict field-based matching cannot decide whether they target meaningful victim-object fields.
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}p{0.43\linewidth}r p{0.32\linewidth}@{}}
    ...
    Primitive-aware records & 97 & Counted cases \\
    Counted primitive sites & 781 & Main sites \\
    AVW/CVW missing destination & 522 & Reported separately \\
    Conservative denominator & 1,303 & Counted + missing \\
    \bottomrule
  \end{tabular}
  \caption{Evaluation dataset funnel. AVW/CVW sites without determined destinations are kept separate because primitive-aware field matching requires a concrete destination field.}
  \label{tab:dataset-funnel}
\end{table}

\noindent\textbf{Reading the counts.} The funnel is 342 stable bugs $\rightarrow$ 115 symbolically reachable bugs $\rightarrow$ 97 primitive-aware records $\rightarrow$ 781 counted primitive sites. The 97-record aggregate is over analyzable explored paths: 227 stable bugs do not reach syscall/interrupt completion under the time budget, and 18 more lack counted primitive-aware records. The 522 separately reported AVW/CVW sites span 47 cases (283 AVW and 239 CVW); if counted conservatively as failures, the OM rate is 15/1,303 and the PM rate is 291/1,303.
...
Table~\ref{tab:primitive-usability-breakdown} reports the same primitive sites under OM and PM. A counted site has enough metadata to apply Table~\ref{tab:primitive-aware-rules}; Excl. records AVW/CVW sites whose write destinations are unavailable. OM is the portable object-matching result, while PM additionally accounts for pointer-related sites under pointer materialization.

\input{generated/primitive_aware_breakdown}



Overall, we observe 781 primitive instances across the 97 bugs with aggregate primitive-aware records. Under OM, 15 sites are candidates: 1 clean site and 14 SyzPass-attributed candidates. Under PM, 276 additional pointer-related sites become candidates (166 AAW, 83 CFH/FPD, and 27 IF$_{sym}$), yielding 291 candidates and 490 remaining failures. This gap highlights how stronger pointer materialization changes the reachable primitive space.
...
At the bug level, 63 of the 97 bugs contain at least one PM-enabled candidate primitive instance. The 14 SyzPass-attributed OM instances consist of 10 IF$_{con}$ sites across 10 bugs and 4 AVW sites across 2 bugs. The IF$_{con}$ sites are double-free-like frees whose residual effects match elastic spray objects such as \texttt{msg\_msg}, \texttt{user\_key\_payload}, or \texttt{simple\_xattr}; the AVW sites have determined destination fields that match useful fields in \texttt{msg\_msg}/\texttt{user\_key\_payload}-style layouts. These matched layouts provide concrete starting points for exploit construction, and we validate representative guidance through the case studies below.
