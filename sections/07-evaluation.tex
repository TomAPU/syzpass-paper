...
For each bug, we run context exploration for 10 hours; afterwards, we allocate 4 hours for primitive discovery and side effect avoidance. Side effects neutralization is performed without a time limit, as it completes deterministically. We use the primitive-aware rules in Table~\ref{tab:primitive-aware-rules}: under the pointer-materialization capability model, pointer-related sites are preserved when their targets are controlled; value-write sites are checked against their determined destination fields; and bounded-write sites must be satisfiable against useful victim-object fields. If a value-write site does not have a determined destination field in the analyzed record, we exclude that site from the aggregate primitive-aware counts instead of inferring a target.
...
\begin{table}[t]
  ...
  \begin{tabular}{llrl}
    ...
    Complete primitive-aware records & cases & 97 & Case denominator for strict counts \\
    Reported primitive sites & sites & 781 & Primitive denominator in Table~\ref{tab:primitive-usability-breakdown} \\
...
Table~\ref{tab:dataset-funnel} makes the counting denominator explicit. The 97 complete records are the case denominator for the strict primitive-aware aggregate, while the 522 excluded AVW/CVW sites are reported separately to avoid treating missing destination information as either successful or failed matching.
...
Overall, we observe 781 primitive instances across the 97 bugs with complete primitive-aware records.
Among them, 291 are usable, and 490 are not usable.
Notably, SyzPass accounts for 14 of the 291 usable instances, all by side-effect neutralization under the strict primitive-aware field model, indicating that some primitive sites become usable only after explicitly reasoning about and mitigating side effects.
Under this strict aggregate, no primitive instance is attributed to avoidance. Avoidance is still part of the exploration and case-study guidance, but the instances that remain in the aggregate either are already clean under the primitive-aware capability model or require neutralization to become usable.

If we focus on higher-risk primitives (OUW, AAW, CFH, and IF), the result reflects our primitive-aware model: pointer-related sites are preserved when their targets are controlled, while residual side effects are handled by avoidance, object matching, or pointer materialization. SyzPass accounts for 10 of 287 usable high-risk primitives. This impact is concentrated in non-symbolic invalid-free sites whose selected operation remains double-free-like and whose residual side effects require explicit object matching.
...
At the bug level, 63 of the 97 bugs contain at least one usable primitive instance.
Moreover, 10 bugs benefit from SyzPass via neutralization. For the remaining 87 bugs, 2 contain only clean primitive instances and thus do not require avoidance or neutralization. Another 51 bugs contain either clean or non-usable instances, and 34 cases have only non-usable primitive instances.

These results collectively indicate that side effects are common among Linux kernel vulnerabilities, and that strict primitive-aware neutralization can still turn otherwise unusable sites into actionable exploit guidance.
...
To understand how SyzPass helps with exploit construction in practice, we conduct three case studies. Specifically, from the 10 bugs that benefited from SyzPass, we selected three that can be triggered by non-root users across different kernel versions and followed SyzPass's guidance to develop exploits. These case studies validate the guidance produced by SyzPass rather than claim fully automatic exploit generation; when an exploit needs a manual step such as selecting a KASLR bypass, we state it explicitly. In this section, we detail these bugs, exploit strategies, and how SyzPass identifies high-risk primitives and resolves side effects among them to facilitate exploit development.
...
Through SyzPass's context exploration, it discovers another syscall \texttt{recvmsg()} that can also be used to trigger the vulnerability. SyzPass does not currently classify information-leak primitives. Based on the vulnerability root cause and the \texttt{recvmsg()} behavior surfaced by SyzPass, we manually identified that this syscall can leak information by reading the \texttt{data} field of \texttt{sk\_buff}. Once lethal side effects are avoided or neutralized, this leak can disclose kernel memory.
