...
Using the original PoC, SyzPass first explores vulnerability contexts to identify alternative syscall sequences that trigger the vulnerability, i.e., \emph{High-Sensitivity Context Exploration}. It then explores execution paths after the initial corruption to search for alternative execution paths to avoid side effects, i.e., \emph{Primitive Discovery \& Side-Effect Avoidance}.
...
\subsection{High-Sensitivity Context Exploration}
...
\begin{figure}[htbp]
  \centering
  \begin{tikzpicture}
    \node[inner sep=0pt, anchor=south west] (img) at (0,0) {\includegraphics[width=\linewidth,trim=0 0 0 5, clip]{figures/symexec.pdf}};
    \begin{scope}[x=0.01\linewidth,y=0.01\linewidth]
      \fill[white] (14,94) rectangle (86,99);
      \node[font=\bfseries\small] at (50,96.5) {Path and Effect Discovery};
      \fill[white] (18,34.5) rectangle (83,40.5);
      \node[font=\bfseries\small] at (50.5,37.5) {Side-Effect Avoidance};
    \end{scope}
  \end{tikzpicture}
  \caption{\label{fig:symexec}Effect Discovery and Side-Effect Avoidance}
\end{figure}
...
\subsection{Side-Effect Neutralization Analysis}
...
\noindent\textbf{4. Object Matching for Side-Effect Neutralization:} Our object-matching process consists of two phases. The primary mode performs primitive-aware matching, requiring both data nodes and pointer dereference chains to align with the candidate object. This is the mode used for the reported $\mathsf{OM}$ results. SyzPass can also run a relaxed diagnostic mode that considers pointer dereference structure alone, which helps exploit developers inspect naturally satisfiable value requirements. For instance, if the data constraint requires some part of the vulnerable memory to be greater than or equal to zero, it is likely satisfiable by many objects naturally.
