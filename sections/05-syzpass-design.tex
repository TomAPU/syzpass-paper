...
SyzPass separates this reasoning from the attacker's capability model. Table~\ref{tab:capability-models} summarizes the two models used throughout the paper: object matching only ($\mathsf{OM}$), our portable baseline, and pointer materialization ($\mathsf{PM}$), a capability-sensitive model for pointer-related primitives.
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}p{0.15\linewidth}p{0.50\linewidth}p{0.25\linewidth}@{}}
    ...
    $\mathsf{PM}$ & $\mathsf{OM}$ plus disclosed kernel addresses and kernel-addressable controlled memory & Pointer-related sensitivity \\
    \bottomrule
  \end{tabular}
...
Recent exploitation techniques motivate $\mathsf{PM}$: software side channels can disclose kernel addresses from unprivileged contexts~\cite{kernelsnitch}, and recent KernelCTF writeups show that attackers can place fake kernel-resident payloads at known kernel addresses with techniques such as NPerm~\cite{kernelctf-nperm}. Fixed or weakly randomized kernel regions have also appeared in real systems, as illustrated by the \texttt{cpu\_entry\_area} randomization issue and its use in exploit construction~\cite{cve20230597,kernelctf-cpuentry}. These techniques make pointer materialization realistic in many environments. SyzPass still retains object matching because such techniques can be unavailable in emulated environments such as QEMU TCG, may depend on deployment-specific side channels, and may be partially mitigated in future kernels. Object matching therefore remains a more portable fallback when a real sprayed object can satisfy the side effect.
