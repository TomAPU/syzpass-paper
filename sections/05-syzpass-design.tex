...
As discussed in \S~\ref{chap:eps}, pointer-dereference-related side effects are particularly critical, as they can lead to kernel crashes if mishandled. Thus, such side effects must be avoided or neutralized before the selected primitive site is accepted by our candidate predicate. In the next section, we discuss how SyzPass searches for neutralization candidates when all explored paths associated with a primitive incur such side effects.
