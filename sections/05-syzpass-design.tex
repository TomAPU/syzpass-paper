...
SyzPass performs neutralization with primitive-aware site reasoning. In our analysis, we model one selected primitive site $s$ per exploit strategy and treat the other effects on the same path as residual effects. We separately determine whether $s$ is preserved as a useful primitive and whether there exists a path where all residual effects can be avoided by path selection or neutralized after they occur. This separation matters because a pointer-shaped primitive can be useful once its pointer target is attacker-controlled, while a value-write or bounded-write primitive must corrupt a useful victim-object field rather than attacker-controlled payload data.
...
The rules below are read in two steps. First, $\mathsf{Preserve}$ checks whether the selected primitive remains useful for the exploit strategy. Pointer-shaped primitives require attacker control of the pointer target; invalid frees are split into symbolic arbitrary-free sites and concrete double-free-like sites; value writes must target useful victim-object fields. Second, $\mathsf{Safe}$ checks whether the other effects on the same path can be avoided or made safe under $\mathsf{OM}$ or $\mathsf{PM}$.
