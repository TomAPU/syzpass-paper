...
To overcome these shortcomings, we design SyzPass, a framework that discovers primitive sites, measures their side effects, and reports primitive-aware candidate guidance for exploit development. In this section, we describe how SyzPass explores alternative contexts, reasons about post-corruption paths, and matches residual side effects against attacker-sprayable object layouts.
...
SyzPass separates this reasoning from the attacker's capability model. Table~\ref{tab:capability-models} summarizes the two models used throughout the paper: object matching only ($\mathsf{OM}$), our portable baseline, and pointer materialization ($\mathsf{PM}$), an upper-bound sensitivity model for pointer-related primitives.
...
\begin{table}[t]
  ...
  \begin{tabular}{lll}
    \toprule
    \textbf{Model} & \textbf{Additional capability} & \textbf{Role in counts} \\
    \midrule
    $\mathsf{OM}$ & Unprivileged trigger, syscall/spray control, heap reclaim by sprayable objects & Baseline; Avoid/Neut. are SyzPass-attributed \\
    $\mathsf{PM}$ & $\mathsf{OM}$ plus disclosed kernel addresses and kernel-addressable controlled memory & Upper-bound sensitivity only for AAW, CFH/FPD, IF$_{sym}$ \\
    \bottomrule
  \end{tabular}}
  \caption{Capability models. $\mathsf{PM}$ does not replace object matching; it reports how results change when pointer materialization is available.}
  \label{tab:capability-models}
\end{table}

Recent exploitation techniques motivate $\mathsf{PM}$: software side channels can disclose kernel addresses from unprivileged contexts~\cite{kernelsnitch}, and recent KernelCTF writeups show that attackers can place fake kernel-resident payloads at known kernel addresses with techniques such as NPerm~\cite{kernelctf-nperm}. Fixed or weakly randomized kernel regions have also appeared in real systems, as illustrated by the \texttt{cpu\_entry\_area} randomization issue and its use in exploit construction~\cite{cve20230597,kernelctf-cpuentry}. These techniques make pointer materialization realistic in many environments, but $\mathsf{PM}$ is an upper-bound post-disclosure/staging sensitivity model rather than a standalone exploit-generation claim or a per-bug deployment claim. SyzPass retains object matching because such techniques can be unavailable in emulated environments such as QEMU TCG, may depend on deployment-specific side channels, and may be partially mitigated in future kernels. Object matching therefore remains a more portable fallback when a real sprayed object can satisfy the side effect.
...
\noindent\textbf{Examples.} For a CFH site, $\mathsf{target}(s)$ must be controlled; under $\mathsf{OM}$, residual dereferences on the chosen path must still match real sprayed-object pointers, while under $\mathsf{PM}$ the selected CFH target itself can be supplied by a materialized pointer chain. For a non-symbolic invalid free where the \texttt{kfree()} argument is the vulnerable object itself, SyzPass marks $\mathsf{DFLike}$ true only when the recovered free argument falls inside the vulnerable-memory range; the site is preserved as IF$_{con}$, but remaining dereferences must satisfy $\mathsf{Safe}$. For CVW/AVW, SyzPass requires a determined $\mathsf{field}(\mathsf{dst}(s))$ before applying Table~\ref{tab:primitive-aware-rules}; otherwise the site is excluded from strict counts.
...
\noindent\textbf{3. Memory Layout Reconstruction for Objects:} We collect objects among existing object databases from prior work to achieve memory layout reconstruction \cite{chen2019slake,chen2020systematic,wang2023alphaexp}, and use them as candidate sprayable layouts. This database supplies matching candidates rather than proving allocator reachability or lifetime control. To pair these objects with a required memory layout, we develop a simple algorithm to construct the object memory layout information through statically analyzing the kernel. In short, we parse the layout information of each structure to pinpoint pointers, then we iteratively generate the layout for pointers to model the pointer-dereference relationship. 
...
\noindent\textbf{4. Object Pairing for Side Effects Neutralization:} Our object pairing process consists of two phases. The first mode performs strict matching, requiring both data nodes and pointer dereference chains to match for precise analysis. All aggregate $\mathsf{OM}$ results in \S\ref{expset} use this strict mode. If data node matching fails, we may also run a relaxed diagnostic mode that only considers pointer dereference structure, but those matches are not counted in the reported primitive-aware results. Relaxed matches are useful because the exploit developer can manually analyze whether a non-user-controllable value requirement is naturally satisfiable. For instance, if the data constraint requires some part of the vulnerable memory to be greater than or equal to zero, it is likely satisfiable by many objects naturally. We leave more precise data constraint solving as future work.
...
A successful match is a candidate neutralization strategy: the residual layout is compatible with a sprayable object, so the dereference side effects can be made memory-safe if the object can be placed with a compatible lifetime. SyzPass reports the primitive, path, and matched object for exploit validation; it does not by itself prove spray reachability, cache compatibility, lifetime control, race/timing stability, semantic object validity, or the final exploit chain.
