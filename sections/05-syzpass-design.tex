...
\noindent\textbf{Neutralization side effects while preserving the primitive.}
...
SyzPass separates primitive-aware reasoning from the attacker's capability model. Table~\ref{tab:capability-models} summarizes the two models used throughout the paper: object matching ($\mathsf{OM}$), our portable baseline, and pointer materialization ($\mathsf{PM}$), a stronger model for pointer-shaped primitive or residual-effect requirements. Intuitively, $\mathsf{OM}$ asks whether a real sprayed kernel object can satisfy the required layout, while $\mathsf{PM}$ asks how the answer changes when the attacker can also place a controlled fake object at a disclosed kernel address.
...
\begin{table}[!h]
  ...
  \begin{tabular}{@{}p{0.15\linewidth}p{0.50\linewidth}p{0.25\linewidth}@{}}
    ...
    \midrule
    $\mathsf{OM}$ & Unprivileged trigger, syscall/spray control, heap reclaim by sprayable objects & Portable baseline \\
    $\mathsf{PM}$ & $\mathsf{OM}$ plus disclosed kernel addresses and attacker-controlled fake objects at known kernel addresses & Pointer-shaped requirements \\
    \bottomrule
  \end{tabular}
...
\noindent\textbf{Why include pointer materialization.}
Recent exploitation techniques motivate $\mathsf{PM}$: software side channels can disclose kernel addresses from unprivileged contexts~\cite{kernelsnitch}, and recent KernelCTF writeups show that attackers can place fake kernel-resident payloads at known kernel addresses with techniques such as NPerm~\cite{kernelctf-nperm}. Fixed or weakly randomized kernel regions have also appeared in real systems, as illustrated by the \texttt{cpu\_entry\_area} randomization issue and its use in exploit construction~\cite{cve20230597,kernelctf-cpuentry}. SyzPass therefore reports both models: $\mathsf{PM}$ captures results under pointer materialization, while $\mathsf{OM}$ remains the portable strategy when a real sprayed object can satisfy the side effect.

$\mathsf{PM}$ focuses on the pointer-safety question after address disclosure and staging are available. Under this model, SyzPass asks whether the selected primitive and residual pointer-shaped effects would remain memory-safe on the explored path. Thus, $\mathsf{PM}$ can supply a valid pointer target for selected pointer-shaped primitives or residual pointer dereferences, but it does not make a value-write primitive useful unless that write still targets a meaningful victim-object field.
