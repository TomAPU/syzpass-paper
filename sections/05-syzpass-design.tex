...
\begin{table}[t]
  ...
  \begin{tabular}{lll}
    \toprule
    \textbf{Selected site $s$} & \textbf{Preserve($s$)} & \textbf{Neutralize($p-s$)} \\
    \midrule
    CFH/FPD & controlled target & avoid; OM; PM for pointer-shaped side effects \\
    AAW & controlled write address & avoid; OM; PM for pointer-shaped side effects \\
    IF$_{sym}$ & symbolic free argument & avoid; OM; PM for pointer-shaped side effects \\
    IF$_{con}$ & double-free-like site & normal object/alias reasoning \\
    OUW/CAW & useful victim-object offset & field matching; OM/PM for residual pointer side effects \\
    CVW/AVW & recovered destination and useful offset & field matching; OM/PM for residual pointer side effects \\
    \bottomrule
  \end{tabular}}
  \caption{Primitive-aware site preservation and side-effect neutralization rules. OM denotes object matching; PM denotes pointer materialization through stronger exploit capabilities.}
