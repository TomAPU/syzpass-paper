...
\noindent\textbf{Coverage and Runtime Scope.} SyzPass relies on Angr for symbolic execution, which is inherently slow and limits the number and depth of paths explored within a time budget. Our strict counts therefore apply to cases with aggregate primitive-aware records and should be read as results over explored paths. Branch-set collection and dereference-depth pruning are engineering mechanisms that keep the guidance pipeline tractable on the studied cases, while future work can improve coverage with faster symbolic execution backends, such as QEMU-based engines~\cite{symfit} or instrumentation-based approaches~\cite{symcc}.
