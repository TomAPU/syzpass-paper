...
In this paper, we take a systematic step toward taming side effects to make exploit construction more practical. Our key observation is that side effects can be handled in two complementary ways. First, attackers can \textit{avoid} side effects by steering execution onto safer bug-triggering paths: changing syscall sequences/arguments or spray contents can induce different bug-triggering behaviors~\cite{lin2022grebe,zou2022syzscope}. Second, when avoidance is impossible, attackers can \textit{neutralize} side effects. For pointer-shaped side effects, neutralization can be achieved either by pairing the dereference chain with compatible sprayed objects, or by materializing valid pointer chains under stronger exploit capabilities such as address disclosure and kernel-addressable controlled memory. For value-write and bounded-write primitives, however, usefulness still depends on corrupting a meaningful victim-object field rather than merely writing into attacker-controlled payload data.
...
To this end, we develop a solution, dubbed SyzPass, to mitigate side effects by automating the aforementioned analysis. It conducts fine-grained per-vulnerability analysis, discovering and modeling unintended memory operation behaviors. We evaluate SyzPass on 115 stable memory corruption bugs collected from Syzbot~\cite{syzbot}. For the 97 bugs with complete primitive-aware records, the strict result set contains 781 primitive instances. Among them, 291 are usable under our primitive-aware rules: 277 are clean, and SyzPass enables 14 additional instances from 10 unique bugs by resolving their side effects. Among 287 usable high-risk primitives (e.g., OOB/UAF write, control-flow hijack, arbitrary-address write, and invalid free), 10 depend on SyzPass neutralization. To verify the guidance produced by SyzPass, we developed three kernel exploits for three different bugs (none of which had publicly available exploits).
...
\squishlist
    ...
    \item \noindent We evaluate SyzPass on real-world vulnerabilities, report strict primitive-aware counts with explicit exclusions, and validate its exploit guidance through three working exploits.
