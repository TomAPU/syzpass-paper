...
To this end, we develop SyzPass, a framework for automated primitive-aware side-effect guidance. It conducts fine-grained per-vulnerability analysis, discovering and modeling unintended memory-operation behaviors. Starting from 342 stable memory-corruption bugs collected from Syzbot~\cite{syzbot}, SyzPass performs primitive-aware analysis for 97 bugs covering 781 primitive instances. Under a pointer-materialization model that reflects stronger modern pointer capabilities, 291 primitive sites across 63 bugs become candidates, spanning nearly two thirds of the analyzed bugs. The portable object-matching subset contributes 15 candidates, including 14 SyzPass-attributed neutralization candidates from 10 unique bugs. To validate selected guidance, we developed three working kernel exploits for three different bugs, covering object-matching neutralization, path guidance, and constraint guidance.
