...
In this paper, we study side-effect handling as candidate guidance for kernel exploit construction. Our key observation is that side effects can be reasoned about in two forms: path guidance and residual-effect neutralization. Path guidance steers execution onto safer bug-triggering paths by changing syscall sequences/arguments or spray contents~\cite{lin2022grebe,zou2022syzscope}. Neutralization lets unavoidable lethal pointer-shaped side effects execute safely by pairing dereference chains with compatible sprayed objects, or by materializing valid pointer chains under stronger exploit capabilities such as address disclosure and kernel-addressable controlled memory. In our strict aggregate, avoidance is credited only when path selection changes a site into an object-matching candidate; otherwise it is reported as case-study path guidance. For value-write and bounded-write primitives, usefulness still depends on corrupting a meaningful victim-object field rather than merely writing into attacker-controlled payload data. We therefore evaluate both a portable object-matching baseline and an upper-bound pointer-materialization sensitivity model, and treat object matching as candidate exploit guidance rather than a proof of full exploitability.
...
To this end, we develop SyzPass, a framework for automated primitive-aware side-effect guidance. It conducts fine-grained per-vulnerability analysis, discovering and modeling unintended memory-operation behaviors. Starting from 342 stable memory-corruption bugs collected from Syzbot~\cite{syzbot}, the strict aggregate applies to 97 bugs with primitive-aware records. It contains 781 counted primitive instances and 522 separately reported AVW/CVW exclusions. Under object matching alone, 15 instances satisfy our candidate predicate: one is already clean and 14 are SyzPass-attributed neutralization candidates from 10 unique bugs. No counted site is credited to avoidance under the strict site-level rule, so aggregate evidence is neutralization-heavy; avoidance is evaluated as path guidance in case studies. Under the stronger pointer-materialization model, 276 additional pointer-related instances become candidates as upper-bound sensitivity results, giving 291 candidates in total. To validate selected guidance, we developed three expert-assisted kernel exploits for three different bugs: one validates a counted object-matching neutralization candidate, and two validate path/constraint guidance outside the strict aggregate.
...
\squishlist
    ...
    \item \noindent We evaluate SyzPass on real-world vulnerabilities, report strict primitive-aware counts with explicit exclusions, and validate selected guidance by building three expert-assisted working exploits, one of which exercises a counted object-matching neutralization candidate.
