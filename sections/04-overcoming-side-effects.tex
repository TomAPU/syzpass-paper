...
Going back to the motivating example and Figure~\ref{fig:motivating}, if the attacker intends to leverage a CFH primitive, either syscall choice may leave an AAW effect as a side effect that needs to be avoided.
...
In the motivating example, consider the strategy that selects the CFH in \texttt{syscall\_B()} as the exploit primitive. Along this path, the AAW at \circled{3} executes before the desired CFH and therefore becomes a residual side effect rather than the selected primitive. Avoidance cannot remove it on this syscall path. Neutralization instead asks whether the AAW can execute without crashing the kernel or destroying the later CFH opportunity.

One way to do so is object matching: the attacker reclaims the freed object with a sprayed object whose layout places a valid writable pointer at the offset dereferenced by the AAW~\cite{SETTLERS66:online,Onedaysh20:online}. The write may still happen, but it lands in memory that is valid and harmless for the chosen exploit strategy.

Under stronger exploit capabilities, neutralization may not need to rely on an existing sprayed kernel object. Recent techniques can disclose kernel addresses and stage attacker-controlled data at kernel addresses that the kernel can legally dereference. In such settings, the attacker can materialize the required pointer chain directly: the residual AAW still executes, but its pointer operands resolve to valid attacker-chosen kernel memory. In both cases, the selected CFH is preserved while the residual AAW no longer blocks execution.
