...
In this section, we illustrate avoidance and neutralization as two generic strategies for reasoning about and handling side effects under the motivating example. We further analyze the limitations of existing primitive discovery frameworks under side effects.
...
Going back to the motivating example and Figure~\ref{fig:motivating}, if the attacker intends to leverage a CFH primitive, either syscall choice may leave an AAW-type residual effect that needs to be avoided.

To do so, the first observation is that \texttt{syscall\_A()} is better than \texttt{syscall\_B()}, since the AAW-type residual effect at \circled{3} is unavoidable when using \texttt{syscall\_B()}. In contrast, the same kind of residual effect at \circled{5} can be avoided if the arguments of \texttt{syscall\_A()} are adjusted properly, i.e., by setting \textcolor{DarkGreen}{\texttt{arg1}} to zero, which can influence the \texttt{if} statement at \circled{4}, and thereby bypassing the residual effect at \circled{5}.
...
Suppose, in the motivating example, the attacker wants to develop an exploit by leveraging an AAW primitive, which means that the CFH-type transfer becomes a residual side effect and should be avoided. Looking at \texttt{syscall\_A()} and \texttt{syscall\_B()}, we can see both have an AAW primitive. However, \texttt{syscall\_A()} is less preferred in this case because the CFH-type residual effect cannot be avoided, unlike in \texttt{syscall\_B()}.


To avoid this CFH-type residual effect in \texttt{syscall\_B()}, the attacker first creates a dangling \textcolor{red}{\texttt{obj}} pointer, then sprays memory so the bytes overlapping \textcolor{red}{\texttt{obj}}\texttt{->field} are \texttt{0x0}. When \texttt{syscall\_B()} is invoked, execution at \circled{7} follows \circled{9} instead of \circled{8}, thus avoiding the residual effect.


% the attacker invokes \texttt{syscall\_B()} and seeks to avoid CFH at \circled{8}. To prevent the kernel code from stepping into \circled{8}, the attacker expect \textcolor{red}{\texttt{obj}}\texttt{->field} be zero. Since \textcolor{red}{\texttt{obj}} is a dangling pointer, its value can be controlled by the attacker via heap spraying. Thus, with this knowledge, the attacker can carefully craft sprayed memory content and nullify \textcolor{red}{\texttt{obj}} to avoid the CFH-type residual effect at \circled{8}
...
Avoidance is preferable when an alternative path can skip the undesired effect entirely. However, some useful primitive paths unavoidably execute another memory effect first. In that case, the attacker may still use the path if the residual effect can be made safe.


In the motivating example, consider the strategy that selects the CFH in \texttt{syscall\_B()} as the exploit primitive. Along this path, the AAW-type effect at \circled{3} executes before the desired CFH and therefore becomes a residual side effect rather than the selected primitive. Avoidance cannot remove it on this syscall path. Neutralization instead asks whether the residual write can execute without crashing the kernel or destroying the later CFH opportunity.

One way to do so is object matching: the attacker reclaims the freed object with a sprayed object whose layout places a valid writable pointer at the dereferenced offset~\cite{SETTLERS66:online,Onedaysh20:online}. The write may still happen, but it lands in memory that is valid and harmless for the chosen exploit strategy.

Under stronger exploit capabilities, neutralization may not need to rely on an existing sprayed kernel object. Recent techniques can disclose kernel addresses and stage attacker-controlled data at kernel addresses that the kernel can legally dereference. In such settings, the attacker can materialize the required pointer chain directly: the residual write still executes, but its pointer operands resolve to valid attacker-chosen kernel memory. In both cases, the selected CFH is preserved while the residual write no longer blocks execution.
...
% \subsection{Side-Effect Avoidance}
% % Fuzz+SymExec
